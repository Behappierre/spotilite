...
export class SpotiLiteApp {
      ...
      prevBtn.onclick = () => this.handlePreviousTrack();
          ...
          if (muteBtn) {
            muteBtn.onclick = () => this.handleMuteToggle();
                     ...
                     // Initialize volume display and sync both sliders
           if (volumeLevel) {
             volumeLevel.textContent = volumeSlider?.value + '%' || '50%';
           }
           
           // Sync both volume sliders on startup
  ...
  private async handleVolumeChange(e: Event): Promise<void> {
    ...
    // Update jukebox volume slider to stay in sync
    const jukeboxVolumeSlider = document.getElementById('jukeboxVolumeSlider') as HTMLInputElement;
    if (jukeboxVolumeSlider) {
      jukeboxVolumeSlider.value = volume.toString();
    }

    // Update jukebox mute button state
    const jukeboxMuteBtn = document.getElementById('jukeboxMuteBtn');
    if (jukeboxMuteBtn) {
      if (volume === 0) {
        jukeboxMuteBtn.classList.add('muted');
      } else {
  ...
  // Sync both volume sliders to ensure they're always in sync
  private syncVolumeSliders(): void {
    const volumeSlider = document.getElementById('volumeSlider') as HTMLInputElement;
    const jukeboxVolumeSlider = document.getElementById('jukeboxVolumeSlider') as HTMLInputElement;
    
    if (volumeSlider && jukeboxVolumeSlider) {
      // Get stored volume preference or default to 50
      const storedVolume = localStorage.getItem('jukebox_volume') || '50';
      const volume = parseInt(storedVolume);
      ...
      this.updateMuteButtonStates(volume);
    }
  }

  // Update mute button states based on volume
  private updateMuteButtonStates(volume: number): void {
    const muteBtn = document.getElementById('muteBtn');
    const jukeboxMuteBtn = document.getElementById('jukeboxMuteBtn');
